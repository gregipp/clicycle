...
- Fixed Escape key hanging in interactive select (bare ESC now cancels like q/Ctrl+C)
- Fixed `cc.select` type stub to match actual `interactive_select` signature
...
- **Table pagination:** Added `page_size` parameter to `cc.table()`. When set,
  tables with more rows than `page_size` display interactive page navigation
  with Next/Previous/Done options. When `None` (default), renders as before.

## [3.3.2] - 2026-02-07

### Fixed

- `Layout.title_align` typed as `Literal["left", "center", "right"]` for mypy
...
- README images now use absolute URLs for PyPI rendering
